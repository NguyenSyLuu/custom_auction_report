...
class Auction extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function loadByProductId($product_id){
        if($this->isWinner($product_id)){
            $customer_id = $this->_bidderFactory->create()->getCurrentCustomer()->getId();
            return $this->getCollection()->getWinAuction($customer_id,$product_id);
        }
        $auction = $this->getResourceCollection()
    ...
    public function getTimeLeft(){
        ...
        $timeLeft = strtotime($this->getEndTime())-strtotime($now);
     ...
     * @return bool
     */
    public function isWinner($productId = null){
        $productId = $productId ? $productId : $this->getProductId();
        $bidder = $this->_bidderFactory->create()->getCurrentBidder()->getWonInfo($productId);
